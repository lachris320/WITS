...
| 2.5 | Session stat tiles ("visitors today" / ~~"this hour"~~) | Not present in legacy `mainwindow.cpp` (no stat-tile UI) — **new in Phase 2**, per design spec | `KioskViewModel::applyStudentLogin` bumps `m_visitorsToday`/`m_visitorsThisHour` (`KioskViewModel.cpp:79-80`); hour counter rolls over on hour change (`tickClock`, `KioskViewModel.cpp:49-55`) | **PASS (automated)** — `tst_kioskviewmodel.cpp::applyStudentSetsCurrentAndBumpsStats`; `tst_qml_kiosk.qml::KioskMain::test_statTilesBound` (stub). See DEFERRED 3.3 below for the real DB-backed aggregate. **INTENTIONAL DEVIATION — record, not parity.** The "This Hour" `LStatTile` was **removed from the kiosk UI** during the owner's Phase 2 GUI-review pass (post-implementation): the surviving tile row is `[ Student Info Card (hero) | Visitors Today ]`, with Visitors Today moved to the right. The design mockup (`Library Kiosk v2.dc.html`) shows three tiles (hero + Visitors Today + This Hour); the shipped kiosk shows two by owner request, so this is a deliberate reduction from the reference, not a missed element. The `m_visitorsThisHour` counter + hourly rollover logic are retained in `KioskViewModel` (still unit-tested) but are no longer surfaced in the UI — kept so a future dashboard/admin surface (Phase 3+) can reuse the session count without re-deriving it. |
...
**RFID on-device verified by:** lachris320 (owner)  **date:** 2026-07-12
