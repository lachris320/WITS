...
Every subsequent drawing call — the header/logo band, the summary tiles, the ranking tables, the chart images (`chartImageSize(usableWidth, ...)` at `:704-706`, `:712`, `:716`, `:735`), the roster table columns (`:786-792`), the footer/signature block (`:868-878`) — is positioned as a fraction of `pageWidth`/`usableWidth`/`pageHeight`/`usableHeight`. `device->pageLayout()` reflects whatever orientation was set on the device **before** `paintReport` runs (`QPdfWriter::setPageOrientation` / `QPrinter::setPageOrientation`, both called before `renderToDevice` — see §5.2/§5.3), so a Landscape device already yields a wider `pageRect`, and every one of those fraction-of-width/height calls reflows automatically. **The renderer requires zero changes.** This is also precisely how the app already handles multi-page pagination and the screen-safe chart-image clamp — geometry read from the device, not assumed — so windowing the orientation fits the renderer's existing design rather than fighting it.
...
The existing `paletteAndChartTypeSettersEmit` test (`:530-543`) is the direct template — it constructs a bare `ReportingViewModel vm;`, asserts a default, wires a `QSignalSpy` on the `*Changed` signal, calls the setter, and asserts both the new value and the spy count. New cases, added alongside it:
...
| `qt-app/quick/tests/tst_reportingviewmodel.cpp` | New cases: default `"Portrait"`; valid `setOrientation` emits + updates; invalid/blank `setOrientation` rejected with **zero** signal emissions; `orientations()` contents; four `pageOrientation()` mapping cases. |
